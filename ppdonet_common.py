...
#Entradas: r_min y r_max, limites radiales del dominio.
#Salidas: funcion transformadora que convierte (r, theta) en (r_scaled, sin(theta), cos(theta)).
# funcion que construye la transformacion periodica usada por la red trunk para tratar theta como variable angular.
def period_transform(r_min, r_max):
    #Entradas: y, tensor de coordenadas con columnas (r, theta).
    #Salidas: tensor con columnas (r_scaled, sin(theta), cos(theta)).
    # funcion que aplica el escalamiento radial y la codificacion periodica angular.
    def transform(y):
        r = y[..., 0:1]
        theta = y[..., 1:2]
        r_scaled = scale_to_one(r, r_min, r_max)
        return torch.cat([r_scaled, torch.sin(theta), torch.cos(theta)], dim=-1)

    return transform


#Entradas: ds_reference, Dataset con coordenadas r y theta; device, dispositivo destino.
#Salidas: coords_tf, r_vals y theta_vals.
# funcion que construye la grilla completa del mapa y la transforma para ser entrada de la red trunk.
def build_coords(ds_reference, device):
    r_vals = ds_reference["r"].values
    theta_vals = ds_reference["theta"].values
    r_grid, theta_grid = torch.meshgrid(
        torch.tensor(r_vals, dtype=torch.float32),
        torch.tensor(theta_vals, dtype=torch.float32),
        indexing="ij",
    )
    coords = torch.stack([r_grid.reshape(-1), theta_grid.reshape(-1)], dim=-1)
    coords_tf = period_transform(r_vals.min(), r_vals.max())(coords).to(device)
    return coords_tf, r_vals, theta_vals
...
class TriDeepONet(nn.Module):
    ...
    #Entradas: u, parametros fisicos del branch; y, coordenadas transformadas del trunk.
    #Salidas: prediccion del campo para cada simulacion y punto de la grilla.
    # funcion que combina branch y trunk por producto punto aprendido y produce el mapa final.
...
#Salidas: modelo TriDeepONet configurado para PPDONet.
# funcion que construye el modelo completo con branch para parametros, trunk para coordenadas y z_net para salida escalar.
def build_ppdonet(
    act="stan",
    stan_beta: float = 0.1,
...
class PPDONetDataset(Dataset):
    #Entradas: Dataset xarray, parametros, coordenadas transformadas, nombre del campo y opciones de preprocesamiento.
    #Salidas: instancia Dataset lista para DataLoader.
    # funcion que prepara pares (parametros, coordenadas, target) para entrenar o evaluar PPDONet.
    def __init__(
        self,
        ds_out,
        params,
        ...
        out_np = ds_out[field_name].values.astype(np.float32)
        ...
        batch_size, n_r, n_theta = out.shape
        self.targets_full = out.reshape(batch_size, n_r * n_theta)

    #Entradas: ninguna.
    #Salidas: numero de simulaciones disponibles.
    # funcion que informa el largo del dataset en cantidad de mapas/simulaciones.
    def __len__(self):
        return self.params.shape[0]

    #Entradas: idx, indice de la simulacion solicitada.
    #Salidas: u, y y target; opcionalmente sampleados en una subgrilla.
    # funcion que entrega un ejemplo de entrenamiento compuesto por parametros branch, coordenadas trunk y valores objetivo.
    def __getitem__(self, idx):
        ...
        ids = self.rng.choice(len(y), size=self.n_points_sample, replace=False)
        ids = torch.tensor(ids, dtype=torch.long, device=self.device)
        return u, y[ids], target[ids]


#Entradas: datasets train/test, parametros, coordenadas, campo objetivo y opciones de preprocesamiento.
#Salidas: DataLoader de entrenamiento y DataLoader de test.
# funcion que crea los loaders para un campo especifico manteniendo la configuracion comun del proyecto.
...
# funcion que reconstruye la arquitectura y carga sus parametros entrenados desde disco.
...
# funcion que evalua velocidad azimutal tras convertir prediccion y target a delta kepleriano.
...
#Entradas: modelo, parametros, coordenadas, Dataset del campo, nombre del campo, grilla y opciones de transformacion.
#Salidas: mapa predicho, mapa verdadero, MSE y R2 para una simulacion.
# funcion que predice un caso individual de test y aplica las mismas transformaciones usadas en entrenamiento/evaluacion.
def predict_single_field(
    ...
    true_map = torch.tensor(ds_field[field_name].values[idx_example], dtype=torch.float32)
    if subtract_background:
        true_map = true_map - true_map[0:1, :]
    if log_target:
        true_map = torch.log10(torch.clamp(true_map, min=1e-12))
    if fargo_kepler_delta:
        true_map = fargo_vtheta_delta(true_map, r_vals)
        pred = fargo_vtheta_delta(pred.reshape(n_r, n_theta), r_vals)

    true_flat = true_map.reshape(-1)
    pred_flat = pred.reshape(-1)
    mse = F.mse_loss(pred_flat, true_flat).item()
    r2 = r2_score(pred_flat, true_flat).item()
    return pred_flat.reshape(n_r, n_theta), true_flat.reshape(n_r, n_theta), mse, r2
